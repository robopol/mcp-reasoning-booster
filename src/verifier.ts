...
function maxSimilarityWithHistory(state: State, text: string): number {
  let maxSim = 0;
...
function avgSimilarityWithHistory(state: State, text: string): number {
  if (!state.steps.length) return 0;
  let sum = 0;
  for (const s of state.steps) sum += jaccardSimilarity(tokenize(s.text), tokenize(text));
  return sum / state.steps.length;
}

// --- Constraint-driven, objective-ish scoring helpers ---
type ExtractedConstraints = {
  singleActionOnly: boolean;
  minimalityDesired: boolean;
  enumeratedFactors: string[];
};

function extractConstraintsFromTask(task: string): ExtractedConstraints {
  ...
  if (enumMatch && enumMatch[1]) {
    ...
    if (parts.length > 2) {
      ...
      if (cands.length >= 3 && cands.length <= 12) enumeratedFactors = cands;
    }
  }
  // Dedup
...
type StepClassification = {
  ...
  mentionedFactors: Set<string>;
  isObservation: boolean; // observe, measure, draw, check, test
  isRelabelOrAssign: boolean; // label, assign, rename
  isDeduction: boolean; // textual conditional reasoning without action
};

function classifyStepAgainstTask(text: string, constraints: ExtractedConstraints): StepClassification {
  ...
  const actionVerbs = new Set<string>(actionList.filter(v => new RegExp(`\\b${v}\\b`, "i").test(text)));
...
function computeObjectiveScores(
  ...
  if (constraints.singleActionOnly) {
    ...
    if (cls.actionVerbs.size > 1 || andCount > 0) constraintPenalty -= 0.2;
    if (cls.mentionedFactors.size > 1) constraintPenalty -= 0.25;
  }

  // Minimality preference: observation beats pure deduction in early steps
  if (constraints.minimalityDesired) {
    if (cls.isObservation) objectiveGain += 0.1;
    else if (cls.isDeduction) constraintPenalty -= 0.05;
  }
  ...
  return {
    scoreStep(task: string, state: State, proposal: StepProposal): StepScoreParts {
      ...
      if (state.steps.length > 0) {
        ...
        if (maxSim >= 0.95) redundancyScore -= 0.5;
        else if (maxSim >= 0.8) redundancyScore -= 0.3;
        else redundancyScore += 0.1;
        if (avgSim < 0.3) rulesScore += 0.2;
        else if (avgSim < 0.5) rulesScore += 0.1;
      } else {
...
function estimateOutcomeEntropy(proposal: StepProposal): number {
  const sources: string[] = [];
  if (typeof proposal.howToVerify === "string") sources.push(proposal.howToVerify);
  if (typeof (proposal as any)?.rationale === "string") sources.push((proposal as any).rationale);
  const joined = sources.join("\n").toLowerCase();
  if (!joined) return 0;

  // Heuristically extract outcome tokens
  // Examples: balance, left tilt, right tilt, pass/fail, case A/B, if/else branches
  const outcomes = new Set<string>();
  if (/balance/.test(joined)) outcomes.add("balance");
  if (/left\s*(tilt|heavy)/.test(joined)) outcomes.add("left");
  if (/right\s*(tilt|heavy)/.test(joined)) outcomes.add("right");
  if (/(heavier|lighter)/.test(joined)) {
    // don't double-count with left/right, keep a generic polarity outcome
    outcomes.add("polarity");
  }
  // Count separators as hints for multiple cases
  const sepCount = (joined.match(/(;|\bor\b|\/)/g) || []).length;
  if (sepCount >= 1) outcomes.add("alt1");
  if (sepCount >= 2) outcomes.add("alt2");
  // If/then patterns imply branching
  if (/\bif\b.*\bthen\b/.test(joined)) {
    outcomes.add("if");
    if (/\belse\b/.test(joined)) outcomes.add("else");
  }

  const n = Math.max(1, outcomes.size);
  if (n <= 1) return 0;
  // Normalized entropy proxy: log2(n) scaled to ~[0, 0.25]
