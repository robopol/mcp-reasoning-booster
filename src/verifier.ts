...
function estimateOutcomeEntropy(proposal: StepProposal): number {
  ...
  const structLabels = new Set<string>();
  for (const o of proposal.verification?.outcomes ?? []) {
    if (o?.label?.trim()) structLabels.add(o.label.trim().toLowerCase());
  }
  ...
  const eoLabels = new Set<string>();
  for (const o of proposal.expectedOutcomes ?? []) {
    if (o?.label?.trim()) eoLabels.add(o.label.trim().toLowerCase());
  }
  if (eoLabels.size >= 2) {
  ...
  if (/(heavier|lighter)/.test(joined)) outcomes.add("polarity");
