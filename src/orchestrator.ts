...
export async function generateCandidateSteps(
  ...
  try {
    ...
    if (prose.length > 0) {
      ...
      if (pr.length > 0) return pr.slice(0, numCandidates);
    }
  } catch {
...
export function applyStep(state: State, chosen: ScoredStep): State {
  ...
  if (state && (state as any) && (state as any).uncertainty && chosen.proposal.verification && chosen.proposal.verification.logFields) {
...
export async function runOneIteration(
  ...
  if (config.executeVerification && chosen?.proposal?.verification?.outcomes) {
    ...
    for (const o of (chosen.proposal.verification.outcomes || [])) {
      if (o?.stateUpdate) notes.push(o.stateUpdate);
    }
    if (notes.length) {
...
async function shallowBeam(
  ...
  const voiProxy: number[] = branches.map(head => {
