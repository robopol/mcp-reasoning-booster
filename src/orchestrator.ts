...
function parseProseToProposals(text: string, k: number): StepProposal[] {
  ...
  for (const block of blocks) {
    ...
    if (textVal && !isPlaceholder(textVal) && textVal.length <= 200 && actionRegex.test(textVal)) {
      const expectedOutcomes: ExpectedOutcome[] | undefined = (() => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (oc && current) {
      ...
      if (rawVal) {
        ...
        if (parts.length) current.expectedOutcomes = parts.map(p => ({ label: p }));
      }
      continue;
    }
...
function getHintBoost(state: State, text: string): number {
  if (!state.hints || state.hints.length === 0) return 0;
  const txtTokens = new Set(simpleTokens(text));
  for (const h of state.hints) {
    const hTokens = new Set(simpleTokens(h));
    let overlap = 0;
    for (const t of hTokens) if (txtTokens.has(t)) overlap++;
    if (overlap >= 2) return 0.1; // conservative boost if at least 2 tokens overlap
  }
  return 0;
}
...
function isLogicPuzzleTFRandomTask(task: string): boolean {
...
function generateLogicPuzzleTFRandomFallback(task: string, k: number): StepProposal[] {
    ...
    if (arr && arr.length) {
      const items: StepProposal[] = arr
        ...
        .map((o: any) => ({
          ...
          expectedOutcomes: Array.isArray(o?.expected_outcomes) ? (o.expected_outcomes as any[]).slice(0, 6).map(v => ({ label: String(v).trim() })).filter(e => e.label.length > 0) : undefined,
        }));
      if (items.length) return items.slice(0, k);
    }
  }
...
export async function generateCandidateSteps(
  ...
  try {
    ...
    const parsed = JSON.parse(jsonText) as Array<{ text: string; rationale?: string; how_to_verify?: string; expected_outcomes?: any[] }>;
    let proposals: StepProposal[] = parsed
      .filter(o => typeof o?.text === "string" && o.text.trim().length > 0)
      .map(o => ({
        text: o.text.trim(),
        rationale: (o.rationale ?? "").trim(),
        howToVerify: (o as any)?.how_to_verify ? String((o as any).how_to_verify).trim() : undefined,
        expectedOutcomes: Array.isArray(o?.expected_outcomes) ? (o.expected_outcomes as any[]).slice(0, 6).map(v => ({ label: String(v).trim() })).filter(e => e.label.length > 0) : undefined,
      }));
...
export function scoreCandidates(
  ...
  const picked: ScoredStep[] = [];
  const maxKeep = proposals.length;
  const distance = (a: string, b: string) => 1 - jaccard(simpleTokens(a), simpleTokens(b));
...
export function applyStep(state: State, chosen: ScoredStep): State {
  ...
  return {
    ...
    steps: [
      ...state.steps,
      { index: nextIndex, text: chosen.proposal.text, rationale: chosen.proposal.rationale, howToVerify: chosen.proposal.howToVerify, expectedOutcomes: chosen.proposal.expectedOutcomes, score: chosen.score },
    ],
  };
}
...
export function summarizeSolution(state: State): string {
  ...
  for (const s of state.steps.slice(-5)) {
    if (s?.expectedOutcomes && s.expectedOutcomes.length > 0) {
      ...
      if (parts.length >= 2) outcomeLines.push(`  -> Outcomes: ${parts.join(" | ")}`);
    }
  }
...
async function shallowBeam(
  ...
  if (branches.length === 0) return top[0]!;

  // Initialize per-branch states after taking the head
  const branchStates: State[] = branches.map(head => applyStep(state, head));
  const cumulative: number[] = branches.map(head => head.score.totalScore);
  ...
  while (totalPulls < budget) {
    ...
    for (let i = 0; i < branches.length; i++) {
      ...
      const ucb = avg + c * Math.sqrt(Math.log(totalPulls + 1 + 1e-9) / n);
      if (ucb > bestUcb) { bestUcb = ucb; bestIdx = i; }
    }

    // Expand selected branch by one step
    const bs = branchStates[bestIdx] as State;
    const proposals = await generateCandidateSteps(
      task,
      bs,
      config.numCandidates,
    ...
    const next = scored[0];
    if (!next) { pulls[bestIdx] = (pulls[bestIdx] ?? 0) + 1; totalPulls++; continue; }
    cumulative[bestIdx] = (cumulative[bestIdx] ?? 0) + next.score.totalScore;
    branchStates[bestIdx] = applyStep(branchStates[bestIdx] as State, next);
  ...
  for (let i = 1; i < branches.length; i++) {
    ...
    if (val > bestScore) { bestScore = val; bestHead = branches[i]!; }
  }
  return bestHead;
}

function updateHintsFromCandidates(state: State, scored: ScoredStep[], maxNew: number = 3): void {
  ...
  for (const s of scored) {
    ...
    if (!txt || !goodVerify) continue;
    // avoid near-duplicates
    const t = simpleTokens(txt);
    let dup = false;
    for (const h of state.hints) {
      if (jaccard(simpleTokens(h), t) >= 0.9) { dup = true; break; }
    }
    if (dup) continue;
    additions.push(txt);
  ...
  if (additions.length) {
    ...
    if (state.hints.length > 20) state.hints = state.hints.slice(-20);
  }
}